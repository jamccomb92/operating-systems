...
File Name    : UltimaWindows.cc
Last Updated :
Description  : Captures the curses windowing library
**********************************************************/
...
void UltimaWindows::write_string_window(WINDOW *Win, string text)
{
    char winBuff[256];
    int nextNewlineRewind = 0;
    int totalNewlineRewind = 0;
    int space_start;
    for (int iter = 0; iter * 255 - totalNewlineRewind < text.length(); iter++)
    {
        for (int i = 0; i < text.length() && i < 255; i++)
        {
            winBuff[i] = text[iter * 255 + i - totalNewlineRewind];
            nextNewlineRewind++;
            if (winBuff[i] == '\n')
            {
                nextNewlineRewind = 0;
        ...
        if (text.length() - iter * 256 > 0)
        {
            winBuff[255 - nextNewlineRewind] = '\0';
        }
        else
        {
            winBuff[text.length() - iter * 256 - nextNewlineRewind] = '\0';
        }
        totalNewlineRewind += nextNewlineRewind;
        wprintw(Win, winBuff); // put text into correct window
        box(Win, 0, 0);        // maintain integrity of the box
        wrefresh(Win);         // draw the window
    }
}

void UltimaWindows::write_string_window(WINDOW *Win, int y, int x, string text)
{
    char winBuff[256];
    int nextNewlineRewind = 0;
    int totalNewlineRewind = 0;
    ...
    for (int iter = 0; iter * 255 - totalNewlineRewind < text.length(); iter++)
    {
        ...
        if (iter == 0)
        {
            mvwprintw(Win, y, x, winBuff); // put text into correct window
            box(Win, 0, 0);                //keep box integrity
            wrefresh(Win);                 // Draw the window
        }
        else
